    ...
    <Stack mt={{ base: '94px', md: '70px' }}>
      ...
      <Stack direction={'row'} display={'flex'} w={'90%'} justifyContent={'center'} m={'50px  auto'}>
        <Stack w={'60%'} gap={10}>
          ...
          <Button backgroundColor={'bgColor.300'} color={'bgColor.100'} w={{ base: 'auto', md: '30%' }} onClick={handleBookTicket}>{event.eventType === 'Free' ? <Text>Buy event ticket for free</Text> : <Text>Buy event ticket for GHS {event.price}</Text>}</Button>
        </Stack>
        <Stack w={'40%'}>
